# Milestone v1.0: ultra-workshop — Autonomous Coding Agent (Tier 2)

**Status:** ✅ SHIPPED 2026-06-01
**Phases:** 1–10 (+ 10.1 inserted gap-closure)
**Total Plans:** 38
**Requirements:** 50/56 satisfied · 1 deferred (REQ-ws-015 MCP) · 5 partial → v1.1 backlog
**Audit:** `.planning/v1.0-v1.0-MILESTONE-AUDIT.md` (integration 10/10, flows 4/4 live-verified)

> Autonomous Telegram-driven build/fix pipeline with HITL, cost circuit breaker,
> 8-scope review wave, per-step execution, and Brain↔Workshop vault signaling —
> live and reproducible on the VPS (root@31.97.130.253). Carried to v1.1:
> REQ-ws-015 (MCP registration, owner-deferred), Mac-side vault round-trip
> (025/027), live LLM planner accuracy (033), Phase 9 caveats (048/050), and the
> Brain-side queue producer (separate ultra-agents-brain repo).
...
- [x] **Phase 8: Specialist Quality Uplift** - In-place soul/discipline/context uplift of all five specialists: lean behavioral souls, build/test verification gate, structured retry feedback, brain reads at planner+reviewer, fix two workshop-fix inconsistencies (completed 2026-05-26, pass_with_caveats)
