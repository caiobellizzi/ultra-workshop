...
- **Repo targeting** — Phase 1 shipped sandbox-only; Phase 6 unlocks an owner-approved active repo registry with HITL before repo mutations or PR creation
...
| L17 | Phase 1 target repo allowlist = `caiobellizzi/test-workshop-sandbox` until owner-approved Phase 6 registry expansion (L17-A) | blast radius |
| L18 | GitHub auth = fine-grained PAT scoped to allowlist until owner-approved Phase 6 registry auth expansion (L18-A); GitHub App remains future upgrade path | GitHub auth |
...
### Owner Amendments — 2026-05-24

These amendments explicitly unlock a follow-up phase after the completed Phase 4 pipeline. They do not retroactively change the Phase 1 safety rationale; they supersede the sandbox-only constraint only for the repo registry workflow described here.
...
- GitHub App auth (replaces expanded fine-grained PAT after Phase 6 proves the registry flow)
